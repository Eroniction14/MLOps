...
In the original repo, the model was trained on the Iris dataset using a Decision Tree classifier.
In this modified version:
1. We use the Breast Cancer dataset from scikit-learn.
2. Only 10 features are used (instead of all 30) for simplicity.
3. The model is a RandomForestClassifier with 100 trees.
4. The API returns human-readable predictions (Malignant or Benign) instead of numeric class labels.
...
└── fastapi_lab1
    ...
    ├── src/
    │   ├── __init__.py
    │   ├── data.py             # loads Breast Cancer dataset (10 features)
    │   ├── main.py             # trains RandomForest model
...
1. First step is to train a Random Forest Classifier (Although you already have model/breast_cancer_model.pkl, let’s create a new model). To do this, move into src/ folder with
    ```bash
    cd src
    ```
...
The **CancerData** class is a [Pydantic model](https://docs.pydantic.dev/latest/concepts/models/) which defines the expected structure of the data for a request body. When you use it as a type annotation for a route operation parameter, FastAPI will perform the following actions:
...
- **Data Validation:** Validate that all 10 fields are present and have correct types. If invalid, FastAPI automatically returns a 422 Unprocessable Entity error response with details.

#### 2. CancerResponse class
...
The **CancerResponse** class is another Pydantic model that defines the structure of the response data for an endpoint. When you specify **response_model=CancerResponse** in a route operation, it tells FastAPI to:
- **Serialize the Output**: Convert the output data to JSON format according to the CancerResponse model.
- **Document the API**: Include the CancerResponse model in the generated API documentation, so API consumers know what to expect in the response.
...
async def read_item(item_id: int):
    item = get_item_by_id(item_id)  # Hypothetical function
    if item is None:
